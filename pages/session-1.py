...
def generate_random_points(num_points):
    return [
        {"id": i, "x": random.uniform(0, 100), "y": random.uniform(0, 100)}
    ...
    for entry in data:
        ...
        if session_1_worldview:
            try:
                ...
                parsed_data.append({"session_1_worldview": session_1_worldview})
            except json.JSONDecodeError:
...
def fetch_data():
    ...
    response = (
        ...
        .select("session_1_values", "session_1_worldview")
        .execute()
    )
...
def intro():
    ...
    st.markdown(
        "## <center>People bring values into the Social Contract.</center>",
        unsafe_allow_html=True,
    )

    st.markdown(
        f"## _Today_ is {now.strftime('%A')}, {now.strftime('%-d')} {now.strftime('%B')} {now.strftime('%Y')} - Session 1",
        unsafe_allow_html=True,
    )
...
def question():
    ...
    dicho = my_create_dichotomy(
        ...
        kwargs={
            ...
            "title": "",
            "name": f"{name}",
            "messages": ["*Zero,* black!", "*One*. White", "*between* gray"],
    ...
    my_create_dichotomy(
        ...
        kwargs={
            ...
            "name": "intuition",
            "messages": [
                "*The future looks* dark like an impending storm",
                "*The future looks* bright and positive",
                "*The future looks* gray like an uncertain mix",
            ],
            # 'inverse_choice': inverse_choice,
            "callback": lambda x: "",
        },
...
"""
    ...
    st.markdown(
        f""" <center>The Athena Collective is a group of 17. 
...
This image tells many stories: it speaks of networks, collaboration, personal agency, shared spaces, and the fluid, evolving nature of social contracts. Each member of the collective shapes the landscape while being shaped by the web they are part of, highlighting the interconnectedness and balance needed to thrive together. Note that connections are _underneath_ the boundaries.</center> """,
        unsafe_allow_html=True,
    )
...
Let's embrace the power of diversity and the richness of each voice in crafting a social contract that speaks to all.
    ...
    st.markdown(
        "Our worldview shapes the very core of our beliefs, actions, and how we interact with others. "
        "By sharing these values, we build the foundation for any social contract."
    )
    # authenticate()
    st.markdown(
        "From philosophical, social, and economic perspectives, our worldview creates boundaries and possibilities."
    )
...
These are core values that guide the construction of a new social contract."""
    ...
    st.warning(
        "Sometimes the pills may not display correctly on Chrome browsers. If you encounter this issue, hit the button below that clears the board."
    )
    ...
    new_value_input = survey.text_input(
        "Add your unique value to the list", key="new_value_input"
    )
    st.button(
        "Visualise the board & Add a new value",
        on_click=add_new_value,
...
In many African cultures, the **Ubuntu** philosophy represents a worldview that emphasises **collective humanity**, interdependence, and shared responsibility. The phrase often associated with Ubuntu is: “**I am because we are**,” highlighting the deep connection between individuals and their communities.
    ...
    inverse_choice = lambda x: resonance_levels.get(
        x,
        {"title": "Invalid", "subtitle": "", "description": "Invalid resonance level."},
    )
    ...
    def update_state(value, element):
        ...
        print(value)
        st.session_state.results.append({"element": element, "result": value})
    ...
    custom_values = [
        value for value, count in value_counts.items() if value.startswith("🎁 ")
    ]
    ...
    for value, count in value_counts.items():
        word_data = {
            ...
            ),  # Random color for now, can be adjusted
        }
    ...
    def prepare_matrix_data(aggregated_data):
        ...
        padded_averages = averages + [None] * (
            matrix_size - len(averages)
        )  # Padding with zero
        ...
        df["Statement"] = df["Hash"].apply(
            lambda h: (
                hash_to_statement.get(h, {}).get("statement", "Unknown")
                if h
                else "Waiting for new data"
            )
        )
        df["Worldview"] = df["Hash"].apply(
            lambda h: (
                hash_to_statement.get(h, {}).get("worldview", "Unknown") if h else ""
            )
        )
        df["Category"] = df["Hash"].apply(
            lambda h: (
                hash_to_statement.get(h, {}).get("category", "Unknown") if h else ""
            )
        )
    ...
    def plot_matrix_scatter(df):
        ...
        fig.update_traces(
            hovertemplate="<b>Hash:</b> %{customdata[0]}<br>"
        ...
        fig.update_traces(
            customdata=df[["Hash", "Worldview", "Category", "Statement"]].values
        )
    ...
    _resonating_statements = {
        key: value for key, value in aggregated_data.items() if value["average"] > 0.7
    }

    # Filter for statements that do not resonate < 0.3
    _non_resonating_statements = {
        key: value for key, value in aggregated_data.items() if value["average"] < 0.3
    }

    # Display results
    st.subheader("Statements that resonate more widely")

    resonating_statements = [
        hash_to_statement.get(h) for h in _resonating_statements.keys()
    ]
    for _statement in resonating_statements:
        if _statement and "statement" in _statement:
            st.markdown(
                f"#### ✨ {_statement['statement']}"
            )  # st.write(resonating_statements)

    st.subheader("\nStatements that dissonate")
    # st.write(_non_resonating_statements)
    non_resonating_statements = [
        hash_to_statement.get(h) for h in _non_resonating_statements.keys()
    ]
    ...
    extracted_data = [
        {"hash": item["element"][1]["hash"], "result": item["result"]}
        for item in results
    ]
