...
def intro():
    ...
    st.markdown(
        "# <center>Structure and Participation</center>", unsafe_allow_html=True
    )

    st.markdown(
        "## <center>Turning a social contract on its head.</center>",
        unsafe_allow_html=True,
    )

    st.markdown(
        f"## _Today_ is {now.strftime('%A')}, {now.strftime('%-d')} {now.strftime('%B')} {now.strftime('%Y')} - Session 2",
        unsafe_allow_html=True,
    )
...
$^1$: Learning Planetizen Manifesto, https://www.learning-planet.org/learning-planetizen-manifesto/
    ...
    strategy = survey.radio(
        "What is a _strategic_ choice?",
        options=["Inclusion", "Exclusion", "I don't know"],
        index=2,
    )
    ...
    if strategy == "Inclusion":
        ...
        inclusion_categories = {
            "0": "All natural elements",
            #    '1': 'All sentient beings',
            "2": "Animals",
            "10": "All animals, plants, mushrooms, and bacteria",
            "30": "All humans",
            "50": "All of us",
        }
        ...
        exclusion_categories = survey.text_area(
            "We are happy to hear. What are criteria for exclusion?",
            id="exclude_criteria_ext",
        )
    else:
    ...
    """
    societal_issues = survey.text_area(
        "Share your thoughts:",
        placeholder="E.g., inequality, climate change, governance...",
        key="symbolic_elements",
    )

    st.divider()
    st.markdown("# Perceptions: Subjective Points of View, an Observatory")
    st.markdown(
        "Does perception shape reality? This observatory platform captures subjective points of view, providing a space where different perspectives on social, political, and economic issues are shared."
    )
    ...
    my_create_dichotomy(
        ...
        kwargs={
            ...
            "name": "intuition",
            "messages": [
                "*Fast transition* rapid and transformative",
                "*Slow* gradual and long",
                "*A mix of the two* with the implications of both",
            ],
            # 'inverse_choice': inverse_choice,
            "callback": lambda x: "",
        },
    ...
    selected_ingredients = st.pills(
        ...
        list(conflict_resolution_elements_split.keys()),
        key="conflict_resolution_elements",
        multiselect=True,
        clearable=True,
        index=None,
    )
    ...
    if selected_ingredients:
        ...
        # ✨
        st.info(
            "📣 " + conflict_resolution_elements_split.get(selected_ingredients[-1], "")
        )
    ...
    survey.button(
        "I Wish To Read and Maybe Share Feedback",
        key="readpaper",
        help="",
        type="primary",
    ...
    survey.text_area(
        "Share your thoughts:",
        placeholder="Describe how you see yourself contributing to the balance and growth of the ecosystem. What actions, responsibilities, or roles do you take on in relation to the world and others around you?",
        key="ecosystem_role",
    )
