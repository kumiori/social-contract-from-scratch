...
def donation():
    ...
    Do you have a favourite number? Try to pick it, it'll bring us good luck.
...
def intro():
    ...
    st.divider()
    st.markdown(f"# <center>We Start With a Wish</center> ", unsafe_allow_html=True)
    st.markdown(f"### _<center>that everything flows smoothly</center>_ ", unsafe_allow_html=True)
...
You've arrived on this page because you're supporting this project by _testing_ our platform. 

Input, output, texts, animations - everything needs a double check. Now it's time to check our payment channels.

Our initiative aims to refresh the way we think about society, governance, and human connection. By supporting us, you're not just contributing to an idea—you're helping build a platform for real exchange. Our mission, with your help, is to challenge the status quo and reimagine the social contract _from scratch_.

Your contribution makes you one of our funding and supporting Philanthropists, and this donation may the beginning of a wider journey
"""
    st_lottie("https://lottie.host/91efca67-fa13-43db-8302-f5c182af8152/ufDyVWvWdR.json")

    """
We are the **Athena Collective**, organising a key panel discussion at the **Europe in Discourse Conference in Athens**, where we'll present a flexible digital platform for governance.
